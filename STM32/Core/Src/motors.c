...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
	if (htim == htim2Ptr) {		// Motor A's interrupt
		uint32_t counter = __HAL_TIM_GET_COUNTER(htim);
		motorAPID->count = (int16_t)counter;
		motorAPID->angle = (int)((motorAPID->count/2)*360/(PULSE_PER_REV));
	}
	if (htim == htim3Ptr) {		// Motor A's interrupt
		uint32_t counter = __HAL_TIM_GET_COUNTER(htim);
		motorBPID->count = (int16_t)counter;
		motorBPID->angle = (int)((motorBPID->count/2)*360/(PULSE_PER_REV));
	}
}
