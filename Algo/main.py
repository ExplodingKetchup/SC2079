from utils.logic import astar_search, greedy_sort, reconstruct_path
from utils.maze import maze_to_graph
from utils.parser import ReadWriteConvert, fix_Commands, write_json
...
def finalmain():
    ...
    print("GOALLIST sorted=", GOALLIST)
    ...
    print("GOALLIST sorted with 3 points", GOALLIST)
    ...
    for i in range(len(GOALLIST) - 1):
        nodesExplored, pathsExplored, nodesProcessed, currentNode = astar_search(
            mazegraph, start=GOALLIST[i], goal=GOALLIST[i + 1]
        )
    ...
    data = ActionsWCamera
    write_json(data, filename="pathForSTM.json")
